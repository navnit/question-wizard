...
export function outlineView(project, selected) {
  ...
  return `<p class="eyebrow">PAPER OUTLINE</p><button class="outline-item cover-item ${selected === null ? 'selected' : ''}" data-action="cover"><span class="outline-number">${icon('file-text')}</span><span>Paper details<small>Cover & instructions</small></span></button><div class="outline-questions">${paper.questions.map((q, i) => `<button class="outline-item ${selected === q.id ? 'selected' : ''}" data-action="question" data-id="${q.id}"><span class="outline-number">${i + 1}</span><span>${escape(q.title || `Question ${i + 1}`)}<small>${q.parts.length} parts · ${q.parts.reduce((s, p) => s + p.marks, 0)} marks</small></span></button>`).join('')}</div>${button('add-question', 'Add question', `class="add-question" ${paper.questions.length >= 100 ? 'disabled' : ''}`, 'plus')}
    ...
    <details class="validation"><summary>${errors.length ? `${errors.length} ${errors.length === 1 ? 'item' : 'items'} to finish` : 'Ready for preview'}</summary>${errors.length ? `<ul>${errors.map(e => `<li>${escape(e)}</li>`).join('')}</ul>` : '<p>Your content and marks are ready.</p>'}</details>`;
}
export function coverView(p) {
  return `<div class="section-heading"><p class="eyebrow">START WITH THE BASICS</p><h1>Paper details.</h1><p class="muted">The school template takes care of the layout.</p></div><div class="form-card">
    ...
    <div class="field-row">${field('Academic year', p.year, 'paper.year', { max: 7, hint: 'For example, 2026-27' })}${field('Term code', p.term, 'paper.term', { max: 10, hint: 'For example, TA-1' })}</div>
...
export function questionView(project, question) {
  ...
  return `<div class="question-heading"><div><p class="eyebrow">WRITE THE CONTENT. WE’LL SET THE PAGE.</p><h1>Question ${index + 1}.</h1></div><div class="question-actions">${iconButton('q-up', 'arrow-up', 'Move question up', index === 0 ? 'disabled' : '')}${iconButton('q-down', 'arrow-down', 'Move question down', index === project.paper.questions.length - 1 ? 'disabled' : '')}${button('duplicate-question', 'Copy', `aria-label="Duplicate question" ${project.paper.questions.length >= 100 ? 'disabled' : ''}`, 'copy')}${button('delete-question', 'Delete', 'class="delete-text"', 'trash')}</div></div>
    <div class="form-card">${field('Question name', question.title, `${prefix}.title`, { max: 100, hint: 'For your outline only; this name is not printed.' })}${richField('Shared instructions (optional)', `${prefix}.context`, false, 'Printed above the first subquestion.')}
      <div class="image-area">${select('Image arrangement', question.imageLayout || 'vertical', `${prefix}.imageLayout`, [['vertical', 'Vertical — stacked'], ['horizontal', 'Horizontal — two per row']])}<p class="arrangement-hint">${question.imageLayout === 'horizontal' ? 'Read left to right, then down. Images are top-aligned and fitted to half the paper width without stretching.' : 'Images are centered, one below another.'}</p><div class="inline-live-preview arrangement-preview" hidden><p>Live paper view</p><canvas class="diagram-live" role="img" aria-label="Live paper image arrangement" hidden></canvas></div>${questionImages(question).map((img, i) => diagramView(project, question, img, i, project.paper.template)).join('')}${button('image', 'Attach a diagram', `class="upload-area" ${questionImages(question).length >= 12 ? 'disabled' : ''}`, 'plus')}<small>Up to 12 diagrams per question. PNG or JPEG, up to 5 MB each. Drag a size slider to see the printed layout.</small></div></div>
    <div class="parts">${question.parts.map((part, pi) => partView(question, part, pi)).join('')}</div>
...
function diagramView(project, question, img, index, templateId) {
  ...
  return `<section class="diagram-card" data-image="${index}" aria-label="Diagram ${index + 1}"><div class="image-heading"><strong>Diagram ${index + 1}</strong><div>${iconButton('image-up', 'arrow-up', `Move diagram ${index + 1} up`, index === 0 ? 'disabled' : '')}${iconButton('image-down', 'arrow-down', `Move diagram ${index + 1} down`, index === questionImages(question).length - 1 ? 'disabled' : '')}${button('remove-image', 'Remove', 'class="text-button"', 'image-off')}</div></div><img class="diagram-thumbnail" src="${escape(source)}" alt="Diagram ${index + 1} attached to question" /><div class="image-settings"><label class="field">Diagram ${index + 1} size<input type="range" data-path="q.${question.id}.images.${index}" min="${minimum}" max="${maximum}" value="${Math.min(maximum, Math.round(img.width))}" /></label>${button('image', 'Replace image', `class="secondary" data-replace="${index}"`)}</div><output class="diagram-dimensions">${imageDimensions(img, question, templateId)}</output><div class="inline-live-preview" hidden><p>Live paper view · updates as you resize</p><canvas class="diagram-live" role="img" aria-label="Live paper layout for diagram ${index + 1}" hidden></canvas></div></section>`;
}
function partView(question, part, pi) {
  const path = `p.${part.id}`, letter = String.fromCharCode(97 + pi), type = responseType(part);
  return `<section class="part-card" data-part="${part.id}" aria-label="Subquestion ${letter}"><div class="part-heading"><h2><span>(${letter})</span> Subquestion</h2><div class="part-actions">${iconButton('part-up', 'arrow-up', `Move subquestion ${letter} up`, `data-id="${part.id}" ${pi === 0 ? 'disabled' : ''}`)}${iconButton('part-down', 'arrow-down', `Move subquestion ${letter} down`, `data-id="${part.id}" ${pi === question.parts.length - 1 ? 'disabled' : ''}`)}${button('delete-part', 'Delete', `class="delete-text" data-id="${part.id}" aria-label="Delete subquestion ${letter}" ${question.parts.length === 1 ? 'disabled' : ''}`, 'trash')}</div></div>
    ${richField('Question prompt', `${path}.text`, true)}${select('Response type', type, `${path}.responseType`, [['written', 'Written response'], ['multiple-choice', 'Multiple choice'], ['true-false', 'True / False']])}<div class="field-row small-fields">${field('Marks', part.marks, `${path}.marks`, { type: 'number', max: 1000 })}${type === 'written' ? field('Answer lines', part.lines, `${path}.lines`, { type: 'number', max: 25, hint: 'Use 0 for no writing lines.' }) : ''}</div>
    ...
    <div class="part-extras">${!part.bank ? button('add-bank', 'Word bank', `class="text-button" data-id="${part.id}"`, 'plus') : ''}${button('add-table', 'Response table', `class="text-button" data-id="${part.id}" ${partTables(part).length >= 12 ? 'disabled' : ''}`, 'plus')}</div></section>`;
}
...
function tableView(part, table, index) {
  return `<section class="content-block" data-table="${index}" aria-label="Response table ${index + 1}"><div class="block-heading"><strong>Response table ${index + 1}</strong><div>${iconButton('table-up', 'arrow-up', `Move table ${index + 1} up`, `data-id="${part.id}" ${index === 0 ? 'disabled' : ''}`)}${iconButton('table-down', 'arrow-down', `Move table ${index + 1} down`, `data-id="${part.id}" ${index === partTables(part).length - 1 ? 'disabled' : ''}`)}${button('remove-table', 'Remove table', `class="text-button delete-text" data-id="${part.id}"`, 'trash')}</div></div>
    <label class="checkbox"><input type="checkbox" data-path="p.${part.id}.table.${index}" ${table.header ? 'checked' : ''} /> First row is a heading</label>
    <div class="table-editor"><table><tbody>${table.rows.map((row, ri) => `<tr>${row.map((text, ci) => `<td><input maxlength="300" aria-label="Row ${ri + 1}, column ${ci + 1}" data-path="p.${part.id}.cell.${index}.${ri}.${ci}" value="${escape(text)}" /></td>`).join('')}</tr>`).join('')}</tbody></table></div>
    <div class="table-buttons">${button('table-row', 'Row', `data-id="${part.id}" ${table.rows.length >= 12 ? 'disabled' : ''}`, 'plus')}${button('table-column', 'Column', `data-id="${part.id}" ${table.rows[0].length >= 5 ? 'disabled' : ''}`, 'plus')}${button('table-remove-row', 'Last row', `class="delete-text" data-id="${part.id}" ${table.rows.length <= 1 ? 'disabled' : ''}`, 'trash')}${button('table-remove-column', 'Last column', `class="delete-text" data-id="${part.id}" ${table.rows[0].length <= 1 ? 'disabled' : ''}`, 'trash')}</div><small>Leave cells empty for students to fill in. Up to 12 rows and 5 columns.</small></section>`;
}
